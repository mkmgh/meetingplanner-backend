                    ...
                    if (err) {
                        ...
                        newUser.save((err, newUser) => {
                            if (err) {
                                ...
                                mailer.autoEmail(newUserObj.email, `<h2>Hi ${newUser.firstName} ${newUser.lastName}, Welcome to Meeting Planner Application</h2><br>
                                <a href='http://meetingplannerapp.emayurm.in.net/email-verify/${newUser.userId}'>Click here to verify yourself</a><br>`, "Email Address Verification");
                                resolve(newUserObj)
                            }
                        })
                    } else {
    ...
    if (check.isEmpty(req.body.email)) {
        ...
        UserModel.findOne({ 'email': req.body.email }, (err, userDetails) => {
            ...
            if (err) {
                ...
                logger.info("user found", "userController: findUser()", 10);
                mailer.autoEmail(req.body.email, `<a href='http://meetingplannerapp.emayurm.in.net/reset-password/${userDetails.userId}'>click here to reset password</a>`, 'Reset Password Request');
